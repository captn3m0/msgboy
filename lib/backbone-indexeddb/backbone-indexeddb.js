(function () { /*global _: false, Backbone: false */
    ...
    Driver.prototype = {
        ...
        execute: function (db, storeName, method, json, options) {
            switch (method) {
            case "create":
                return this.write(db, storeName, json, options);
            case "read":
                if (json instanceof Array) {
                    return this.query(db, storeName, options); // It's a collection
                } else {
                    return this.read(db, storeName, json, options); // It's a Model
                }
            case "update":
                return this.write(db, storeName, json, options); // We may want to check that this is not a collection
            case "delete":
                return this.delete(db, storeName, json, options); // We may want to check that this is not a collection
            default:
                // Hum what?
            }
        },
    ...
    ExecutionQueue.prototype = {
        ...
        next: function() {
            var message = this.stack.shift();
            if(message) {
                message[2].prev_success = message[2].success;
                message[2].prev_error = message[2].error;
                message[2].success = function(result) {
                    message[2].prev_success(result);
                    this.next();
                }.bind(this);
                message[2].error = function(error) {
                    message[2].prev_error(error);
                    this.next();
                }.bind(this);
