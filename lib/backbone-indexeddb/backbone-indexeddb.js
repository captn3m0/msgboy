            ...
            if (typeof (readCursor) == "undefined" || !readCursor) {
                ...
                readCursor.onsuccess = function (e) {
                    ...
                    if(!cursor) {
                        ...
                        if(options.from && options.from.id != cursor.value.id && !from_found) {
                            ...
                            // if(bounds) {
                            //     cursor.continue(bounds.upper + 1); /* We need to 'terminate' the cursor cleany, by moving to the end */
                            // } else {
                                cursor.continue(10000); /* We need to 'terminate' the cursor cleany, by moving to the end */
                            // }
                        }
                            ...
                            if(options.from && options.from.id == cursor.value.id) {
                                from_found = true; // Mark the after as found.
                                cursor.continue(); 
                            }
                            else if(options.to && options.to.id == cursor.value.id) {
                                to_found = true; // Mark the after as found.
                                cursor.continue(10000); /* We need to 'terminate' the cursor cleany, by moving to the end */
                            }
                            else {
                                processed++;
