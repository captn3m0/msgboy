...
var Store = function(name) {
  this.name = name;
  var collectionStore = localStorage.getItem(this.name);
...
_.extend(Store.prototype, {
  ...
  findAll: function() {
    return _.map(this.members, this.find);
  },
  ...
  destroy: function(model) {
    ...
    if(idx!=-1) {
      ...
      this.save();
      localStorage.setItem(this.name + ":" + model.id, null);
    }
    return model;
  }
...
Backbone.sync = function(method, model, success, error) {
  ...
  if(model.storeName) {
    var store =  new Store(model.storeName);
